...
/**
 * MIRC anonymizer functions. These static methods provide the data modification
 * functions used by the MIRC anonymizers. See the
 * <a href="http://mircwiki.rsna.org">MIRC wiki</a> for more more information.
 */
public class AnonymizerFunctions {

  /**
   * Look up a String in a local unencrypted Properties object. The Properties
   * table keys must be of the form <b>keyType/key</b>.
   * 
   * @param table
   *        the lookup table
   * @param keyType
   *        any name relating keys of a particular category, for example "ptid".
   * @param key
   *        the value of the keyType to be looked up in the table.
   * @return the value in the table corresponding to keyType/key.
   * @throws Exception
   *         if the requested value cannot be found in the table.
   */
  public static String lookup(Properties table, String keyType, String key) throws Exception {
    if (table == null)
    ...
    if (value == null)
      throw new Exception("missing key (" + key + ") in lookup table");
    ...
    if (indirects <= 0)
      throw new Exception("more than " + maxIndirects + " indirects");
    return value;
  }

  /**
   * Generate a replacement string for a text string using an IntegerTable. The
   * replacement string is an integer numerical string. Replacement strings are
   * based on the keyType, so text strings of different keyTypes are handled
   * independently.
   * 
   * @param table
   *        the IntegerTable to be used in assigining replacement strings.
   * @param keyType
   *        any name relating text strings of a particular category, for example
   *        "ptid".
   * @param text
   *        the value for which a replacement string is to be created.
   * @param width
   *        the minimum width of the replacement string.
   * @return the replacement string for keyType/text.
   * @throws Exception
   *         if the replacement string cannot be created.
   */
  public static String integer(IntegerTable table, String keyType, String text, int width) throws Exception {
    if (table == null)
  ...
  /**
   * Generate the initials of a patient from the contents of a string formatted
   * as "last^first^middle".
   * 
   * @param name
   *        the name in the format of a DICOM PatientName element.
   * @return the patient's initials in upper case (e.g., FML), or "X" if the
  ...
  /**
   * Generate a numeric replacement for a patient name from the contents string
   * formatted as "last^first^middle".
   * 
   * @param string
   *        the name to hash.
   * @param length
   *        the number of characters (maximum) in the output string.
   * @param wordCount
   *        the number of words to include in the hash. This can be used to
   *        exclude, for example, the middle name or initial.
   * @return a numeric string consisting of length characters, the result of
   *         hashing the first wordCount words of the string.
   */
  public static String hashName(String string, int length, int wordCount) throws Exception {
    if (wordCount <= 0)
   ...
   * 
   * @param ageString
   *        the age string to round.
   * @param width
   *        the width of each bin
   * @return the age rounded to bins of size width, where the first bin is
   *         centered on zero. If the input string ends with alphabetic
   *         characters, they are appended to the result. Thus, if the function
   *         call is ("27y",5), the result String is 25y. If the length of the
   *         result string (with suffix characters) is odd, a leading "0" is
   *         prepended.
   * @throws Exception
   *         if the age does not parse.
   */
  ...
  /**
   * Re-identify a patient by hashing the combination of a site ID and a patient
   * ID for the site.
   * 
   * @param siteid
   *        the ID of the site which generated the ptid.
   * @param ptid
   ...
   *         if the hash fails.
   */
  public static String hashPtID(String siteid, String ptid) throws Exception {
   ...
   * @return the MD5 hash of the combination of the siteid and ptid as a numeric
   *         string with the .
   * @throws Exception
   *         if the hash fails.
   */
  public static String hashPtID(String siteid, String ptid, int maxlen) throws Exception {
    if (siteid == null)
      siteid = "";
    else
      siteid = siteid.trim();
  ...
  /**
   * Generate an MD5 hash of an element text, producing a base-10 digit string.
   * 
   * @param string
   *        the string to hash.
  ...
  private static byte[] getEncryptionKey(String keyText, int size) throws Exception {
    ...
    while (keyText.length() < requiredChars)
      keyText += nonce;
   ...
   * 
   * @param date
   *        the date in DICOM format.
   * @param increment
   *        the number of days to increment the date. Positive increments make
   *        the date later; negative increments make it earlier.
   * @return the incremented date string.
   * @throws Exception
   *         if the date is in an illegal format.
   */
  public static String incrementDate(String date, long increment) throws Exception {
    ...
    return intToString(dateCal.get(Calendar.YEAR), 4) + intToString(dateCal.get(Calendar.MONTH) + 1, 2) + intToString(dateCal.get(Calendar.DAY_OF_MONTH), 2) + ((date.length() > 8) ? date.substring(8) : "");
  }
   ...
   *        the date in DICOM format.
   * @param y
   *        the value to which to reset the year field (-1 means to leave it
   *        alone).
   * @param m
   *        the value to which to reset the month field (-1 means to leave it
   *        alone).
   * @param d
   *        the value to which to reset the day field (-1 means to leave it
   *        alone).
  ...
  public static String modifyDate(String date, int y, int m, int d) throws Exception {
    ...
    if (y < 0)
      y = dateCal.get(Calendar.YEAR);

    if (m < 0)
      m = dateCal.get(Calendar.MONTH);
    else
      m--;

    if (d < 0)
      d = dateCal.get(Calendar.DAY_OF_MONTH);
  ...
  /**
   * Create a new UID from the current time and an incrementing value,
   * prepending a prefix.
   * 
   * @param prefix
   *        the prefix for the new UID.
  ...
  public static String newUID(String prefix) throws Exception {
    ...
    // Now make the UID
    return prefix + Long.toString(time) + "." + Integer.toString(disambiguator++);
  }

  /**
   * Create a new UID by hashing an old UID and prepending a prefix to the
   * hashed value.
   * 
   * @param prefix
   *        the prefix for the new UID.
  ...
  public static String hashUID(String prefix, String uid) throws Exception {
    ...
    if (newuid.length() > 64)
      newuid = newuid.substring(0, 64);
    return newuid;
  }

  /**
   * Get a string containing the current time, with a specified separator.
   * 
   * @param separator
  ...
  public static String time(String separator) {
    ...
    return intToString(now.get(Calendar.HOUR_OF_DAY), 2) + separator + intToString(now.get(Calendar.MINUTE), 2) + separator + intToString(now.get(Calendar.SECOND), 2);
  }

  /**
   * Get a string containing the current date, with a specified separator.
   * 
   * @param separator
   *        the separator for year, month, and day.
   * @return the current date.
   */
  public static String date(String separator) {
    Calendar now = Calendar.getInstance();
    return intToString(now.get(Calendar.YEAR), 4) + separator + intToString(now.get(Calendar.MONTH) + 1, 2) + separator + intToString(now.get(Calendar.DAY_OF_MONTH), 2);
  }
